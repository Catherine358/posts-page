...
export const useStorePosts = create<PostsStore>()(
  persist(
    (set, get) => ({
      favPosts: [],
      temporaryFavPosts: [],
      ...
      toggleFavPosts: (postId) =>
        set((state) => ({
          favPosts: state.favPosts.includes(postId)
            ? state.favPosts.filter((prevPostId) => prevPostId !== postId)
            : [...state.favPosts, postId],
        })),
